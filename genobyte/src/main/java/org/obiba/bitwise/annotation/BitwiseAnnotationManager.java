...
class BitwiseAnnotationManager {
  ...
  BitwiseAnnotationManager(Class<?> c) {
    ...
    if(c.isAnnotationPresent(BitwiseRecord.class) == false) {
      throw new InvalidAnnotationException("Class ["+c.getName()+"] should be annotated with the @BitwiseRecord annotation.");
    }
    ...
    for (int i = 0; i < properties.length; i++) {
      ...
      Annotation[] storingAnn = {
        currentField != null ? currentField.getAnnotation(Stored.class) : null,
      ...
      for (int j = 0; j < storingAnn.length; j++) {
        ...
        if (currentAnn != null) {
          ...
          if (foundAnn == true) {
            throw new InvalidAnnotationException("Class ["+c.getName()+"] declares a property ["+property.getName()+"] that has more than one of these annotations (@Stored or @NotStored).");
          }
      ...
      if(stored != null && stored.unique() == true) {
        if(uniqueFieldDescriptor_ != null) {
          throw new InvalidAnnotationException("Class ["+c.getName()+"] should can only have one Stored annotation with attribute uniqe == true.");
        }
    ...
    if (bitwiseAnnotation != null) {
      ...
      for (int i=0; i<dictAnn.length; i++) {
        ...
        if (dictionaries_.get(dictName) != null) {
          throw new InvalidAnnotationException("There are two dictionaries with name [" + dictName + "] in the class hierarchy.");
        }
