  ...
  private DictionaryMetaData createDictionaryFromAnnotation(BitwiseAnnotationManager ba, DictionaryDef pDictionary) {
    ...
    if (dictClass != void.class) {
      if (dictClassName != null && dictClassName.equals("") == false) {
        throw new InvalidAnnotationException("Class ["+ba.getRecordClass().getName()+"] can only have one of these attributes: dictionaryClass, dictionaryClassName.");
      }
      newDict.setClass(dictClass.getName());
    }
    else {
      if (dictClassName.equals("")) {
        throw new InvalidAnnotationException("Class ["+ba.getRecordClass().getName()+"] should have at least one of these attributes: dictionaryClass, dictionaryClassName.");
      }
