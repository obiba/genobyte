...
public class CsvReport implements StatsDigester {
  ...
  private AbstractField getField(StatsPool<?, ?> pPool, ReportField pRf) {
    ...
    if (pRf.getType().equals("pool")) {
      ...
      if (field == null) {
        throw new IllegalArgumentException("The field [" + pRf.getName() + "] could not be found in the pool of statistics.");
      }
    }
    else {
  ...
  public void digest(StatsPool<?, ?> pPool) {
    ...
    for (int currentRecord=pPool.getRecordMask().next(0); currentRecord!=-1; currentRecord=pPool.getRecordMask().next(currentRecord+1)) {
