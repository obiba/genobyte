...
public class LargeGZIPInputStream extends GZIPInputStream {
  ...
  public int read(byte[] buf, int off, int len) throws IOException {
    ...
    try {
      ...
      if(!(nbReadBytes_ > 4*1024*1024*1024 && e.getMessage() != null && e.getMessage().startsWith("Corrupt GZIP trailer")))
        throw e;
    }
    return retVal;
  }
    ...
    for(int idx=0; idx < args.length; idx++) {
      ...
      try {
        ...
        while(true) {
          ...
          if(read != read2)
            System.out.println("Read different. New Impl: " + read + " >original: " + read2);
          ...
          for(int i =0; i < read; i++) {
            if(bytes[i] != bytes2[i])
              System.out.println("Bytes at " + read + " are different. New Impl: " + bytes[i] + " >original: " + bytes2[i]);
          }
        }
      }
      catch(Exception e) {
