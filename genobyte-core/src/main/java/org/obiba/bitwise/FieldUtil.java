...
class FieldUtil {
  ...
  void clear(BitVector clear) {
    FieldDtoDao fieldDtoDao = (FieldDtoDao)KeyedDaoManager.getInstance(store_.getDaoKey()).getDao(FieldDtoDao.class);
    BitVectorDtoDao bitVectorDtoDao = (BitVectorDtoDao)KeyedDaoManager.getInstance(store_.getDaoKey()).getDao(BitVectorDtoDao.class);
    ...
    for(int i = 0; i < bits.length; i++) {
      ...
      if(bitIndex != -1) {
        ...
        if(vectors[i].size() != dto.getSize()) {
          throw new IllegalStateException("Cannot open field ["+name+"]: vector ["+i+"] size ["+vectors[i].size()+"] does not match field size ["+dto.getSize()+"]");
        }
      }
    }
  ...
  void save(Field f) {
    if(f.isDirty()) {
      ...
      for(int i = 0; i < vectors.length; i++) {
        ...
        if(v != null) {
          if(v.size() != f.getSize()) {
            throw new IllegalStateException("Field ["+f.getName()+"] vector id ["+f.getDto().getBitIndex()[i]+"] size ["+v.size()+"] does not match field size ["+f.getSize()+"]");
          }
