...
public class WildcardQuery extends Query {
  ...
  public QueryResult execute(BitwiseStore store) throws QueryExecutionException {
    ...
    if(myDict instanceof WildcardDictionary == false) {
      // TODO
      throw new QueryExecutionException("Invalid Dictionary type. Expected WildcardDictionary instance for dictionary ["+myDict.getName()+"]");
    }
    ...
    if (valuePre_ != null) {
      ...
      BitVector encodedValuePre = wildcardDict.partialLookupLeft(valuePre);   //We don't want the partial query term to have an "End of string" character
