...
public class PrintHistoryCommand implements CliCommand {
  ...
  public boolean execute(Option opt, CliContext context) throws ParseException {
    ...
    int storeNameWidth = Math.max(context.getStore().getAssayRecordStore().getStore().getName().length(), context.getStore().getSampleRecordStore().getStore().getName().length());
    ...
    for(int i = 0; i < qh.size(); i++) {
      ...
      printer.printLine(Integer.toString(i+1), qe.getStore().getStore().getName(), Integer.toString(qe.count()), qe.getQuery());
    }
    return false;
  }
