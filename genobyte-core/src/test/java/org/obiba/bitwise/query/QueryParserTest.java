...
import org.obiba.bitwise.mock.query.MockBooleanQuery;
import org.obiba.bitwise.mock.query.MockDiffQuery;
import org.obiba.bitwise.mock.query.MockFieldValueQuery;
import org.obiba.bitwise.mock.query.MockNotQuery;
import org.obiba.bitwise.mock.query.MockQueryFactory;
import org.obiba.bitwise.mock.query.MockWildcardQuery;
import org.obiba.bitwise.query.BooleanQuery;
...
public class QueryParserTest extends TestCase {
  ...
  public void testMultipleFieldQuery() {
    try {
      ...
      Query result = new MockBooleanQuery(BooleanQuery.Operator.OR, q1, new MockBooleanQuery(BooleanQuery.Operator.AND, new MockNotQuery(q2), q3));
      assertEquals(result, q);

      q = qp.parse("field1:value OR NOT (field2:value AND field3:0.99)");
      result = new MockBooleanQuery(BooleanQuery.Operator.OR, q1, new MockNotQuery(new MockBooleanQuery(BooleanQuery.Operator.AND, q2, q3)));
      assertEquals(result, q);

      q = qp.parse("NOT field1:value AND NOT field2:value AND field1 DIFF field2");
      result = new MockBooleanQuery(BooleanQuery.Operator.AND, new MockNotQuery(q1), new MockBooleanQuery(BooleanQuery.Operator.AND, new MockNotQuery(q2), new MockDiffQuery("field1", "field2")));
      assertEquals(result, q);
